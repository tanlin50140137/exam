...
{	
	...
	if( $ordersn!=null && $username!=null ) 
	{
		...
		if( !empty( $timus ) )
		{
			...
			$kaotimodule  = '<div class="exam_rembershdiv4">计时'.$times.'小时</div>'; 
	    	...
	    	for($i=1;$i<=$timus['count'];$i++)
	    	{
	    		if( $index[$i-1]['index'] != $_POST['index'] )
		    	{
			    	...
			    	{
			    		$kaotimodule .= '<dd class="exam_rembershdd6 exam_rembershdd6_yes" onclick="exam.PageNumberVIP('.$index[$i-1]['type'].','.$index[$i-1]['tb'].','.$i.');" title="点击快速修改">'.$i;
		    	...
		    	else 
		    	{
		    		if( $index[$i-1]['index'] == $i )
			    	{
			    		$kaotimodule .= '<dd class="exam_rembershdd6 exam_rembershdd6_yes2" onclick="exam.PageNumberVIP('.$index[$i-1]['type'].','.$index[$i-1]['tb'].','.$i.');" title="点击快速修改">'.$i;
			    		if( $index[$i-1]['rightkey'] == '放弃' )
			    		{
			    			$kaotimodule .= '<span class="exam_rembershdd6_span"><font color="red"><b>'.$index[$i-1]['rightkey'].'</b></font></span>';
			    		}
...
function ChargeEditionVIP()
{
	...
	$_SESSION['ChARGEEDITIONVIPS_1'][$xb][$bel][$row['pid']][$type][$tb] = $rightkey;	
 	$_SESSION['ChARGEEDITIONVIPS_4'][$xb][$bel][$row['pid']][$index-1] = array('index'=>$index,'rightkey'=>strtoupper($rightkey),'type'=>$type,'tb'=>$tb);
...
function giveupVIP()
{
	...
	$_SESSION['ChARGEEDITIONVIPS_1'][$xb][$bel][$row['pid']][$type][$tb] = SHOWCENTRENO_16;		
 	$_SESSION['ChARGEEDITIONVIPS_4'][$xb][$bel][$row['pid']][$index-1] = array('index'=>$index,'rightkey'=>SHOWCENTRENO_16,'type'=>$type,'tb'=>$tb);
 	$_SESSION['ChARGEEDITIONVIPS_index'][$xb][$bel][$row['pid']] = $index;
