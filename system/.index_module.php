...
function tollbox_ps()
{
	...
	if( !empty( $rule1['r_name'] ) )
	{
		...
		$html .= '</div></form>';
	    $html .= '<div style="margin:1rem;padding:0 0.8rem;text-align:right;"><span id="pay_errortxt" style="float:left;color:#e80f59;font-size:xx-small;line-height:40px;"></span><input type="image" src="'.$bottonImg.'" width="70" onclick="exam.sendpays(\'FrmListID\');"/></div>';
	    $html .= '</div>';	
	}
...
function payments()
{
	...
	if( $data['username'] != '' )
	{
		...
		if( !empty( $row ) )
		{
			db()->update(PRE.'paymentform', array('price'=>$data['price']), array('username'=>$data['username'],'centreno'=>$data['centreno']));
		}
		else 
		{
